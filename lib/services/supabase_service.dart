...
class SupabaseService {
  ...
  /// If a record exists, it updates the status; otherwise, it inserts a new record.
  static Future<bool> setAttendanceStatus({
    ...
    try {
      ...
      debugPrint('[Supabase] setAttendanceStatus failed: $e');
      return false;
    }
  }
  ...
  static Future<List<SessionModel>> getSessionsForSubjectReport({
    ...
    try {
      ...
      final classInList = classIds.map((id) => '"$id"').join(',');
      final data = await client
          .from('sessions')
          .select('*, classes(name), subjects(name)')
          ...
          .or('class_id.in.($classInList),class_ids.ov.{$classInList}')
